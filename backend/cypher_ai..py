...
def basico(mensagem):
    ...
    if "gleyson" in mensagem or "gleyso" in mensagem or "william" in mensagem:
        return """Willian Gleyson da Silva“Borracheiro” Albuquerque é um jovem mecânico tianguaense responsável por elevar Tianguá ao posto de maior centro mecânico da Serra da Ibiapaba.
    Renomado por sua genialidade na criação de graxas, Gleyson estudou profundamente os segredos da lubrificação, dominando cada fórmula, textura e cheiro de óleo queimado.
Após anos de dedicação, ele alcançou um nível jamais visto entre os borracheiros comuns, tornando-se conhecido como o Último Mestre da Graxa, aquele que domina os quatro elementos da oficina:
pneu, chave de roda, compressor e graxa. Um verdadeiro Avatar da borracharia moderna."""
